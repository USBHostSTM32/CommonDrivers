...
CanManager_StatusTypeDef can_manager_init(can_manager_t *can_manager,
	...
	if ((can_manager != NULL) && (config != NULL) && (config->hcan != NULL)) {
		...
		if ((HAL_CAN_Start(can_manager->config->hcan) == HAL_OK)
				&& (HAL_CAN_ActivateNotification(can_manager->config->hcan,
						can_manager->config->auto_data_feedback_rx_interrupt) == HAL_OK)) {
...
CanManager_StatusTypeDef can_manager_auto_control_tx(can_manager_t *can_manager,
	...
	if ((can_manager != NULL) && (can_data != NULL)) {
		...
		if (HAL_CAN_IsTxMessagePending(config->hcan,
				config->auto_control_tx_mailbox) == 0U) {
			can_manager->n_aborts = 0U;
			...
			if (can_manager->n_aborts < CAN_MANAGER_MAX_ABORTS){
				status = CAN_MANAGER_OK;
			}
		}
	}
