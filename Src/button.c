...
Button_StatusTypeDef button_is_pressed_long(button_t *button) {
    ...
    if (button != NULL) {
        ...
        case PRESSING:
            ...
            } else if ((HAL_GetTick() - button->start_pressing_time) >= BUTTON_LONG_PRESSING_WAITING_TIME) {
...
Button_StatusTypeDef button_is_pressed_level(button_t *button) {
    ...
    if (button != NULL) {
        if ((button->actual_raw_state == BUTTON_PRESSED) && (button->previous_raw_state == BUTTON_NOT_PRESSED)) {
            button->state = ((!button->state) & BUTTON_8BIT_MASK);
        }
        status = BUTTON_OK;
    }
