...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  interface = USBH_FindInterface(phost, phost->pActiveClass->ClassCode, HID_NONE_BOOT_CODE, 0xFFU);
  ...
  else if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol == HID_NONE_BOOT_CODE)
  {
	  ...
	  } else*/ if ((phost->device.DevDesc.idVendor == T818_ID_VENDOR) && (phost->device.DevDesc.idProduct == T818_ID_PRODUCT)) {
