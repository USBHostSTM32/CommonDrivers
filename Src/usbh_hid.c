...
extern USBH_StatusTypeDef USBH_HID_T818Init(USBH_HandleTypeDef *phost); //ADDED TO DEFAULT STM LIBRARY
...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost) {
	...
	phost->pActiveClass->pData = (HID_HandleTypeDef*) USBH_malloc(
	...
	if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol
			== HID_KEYBRD_BOOT_CODE) {
		USBH_UsrLog("KeyBoard device found!");
		HID_Handle->Init = USBH_HID_KeybdInit;
	} else if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol
			== HID_MOUSE_BOOT_CODE) {
		USBH_UsrLog("Mouse device found!");
		HID_Handle->Init = USBH_HID_MouseInit;
	} else if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol
			== HID_NONE_BOOT_CODE) {
		USBH_UsrLog("None boot code device found!");
		/*if(phost->device.DevDesc.idVendor == T818_ID_VENDOR){
		 USBH_UsrLog("Chinese joystick found.");
		 HID_Handle->Init = USBH_HID_T818Init;
		 } else*/if ((phost->device.DevDesc.idVendor == T818_ID_VENDOR)
				&& (phost->device.DevDesc.idProduct == T818_ID_PRODUCT)) {
			USBH_UsrLog("Thrustmaster T818 found.");
			HID_Handle->Init = USBH_HID_T818Init;
		} else {
			USBH_UsrLog("Device not supported.");
		}
	} else {
	...
	for (num = 0U; num < max_ep; num++) {
		if ((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[num].bEndpointAddress
				& 0x80U) != 0U) {
			HID_Handle->InEp =
